...
export default function AlternativeProduct({
  ...
  const [alternativeProductAvailable, setAlternativeProductAvailable] =		useState(false);

  const [alternativeProductJson, setAlternativeProductJson] =		useState(sampleProducts);

  const waitingForAlternativeProducts = async (
    firstCatego: productAttributes['firstCatego'],
  ): Promise<alternativeAttributes> => {
    const alternativeProductJson: alternativeAttributes =			await retrieveAlternativeInformation(firstCatego);
